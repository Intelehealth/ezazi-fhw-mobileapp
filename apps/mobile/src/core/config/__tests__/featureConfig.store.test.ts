import { configStorage } from '@/core/services/storage/config-storage';
import { fetchPublishedConfig } from '@/core/config/config.api';
import { defaultConfigResponse, defaultFeatureFlags } from '@/core/config/config.types';
import type { ConfigResponse } from '@/core/config/config.types';

jest.mock('@/core/services/storage/config-storage', () => ({
  configStorage: { load: jest.fn(), loadVersion: jest.fn(), save: jest.fn() },
}));
jest.mock('@/core/config/config.api', () => ({
  fetchPublishedConfig: jest.fn(),
}));
jest.mock('@/core/utils/logger', () => ({ logger: { debug: jest.fn() } }));
